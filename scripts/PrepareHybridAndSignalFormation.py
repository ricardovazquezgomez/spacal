...
def main(args):
    ...
    for inputFolder in inputFolderList:
        ...
        with open(run_script, "w") as output:
            ...
            output.write(buildFolder + "propagateHybrid -i $INPUT_F -c $CALIBRATION -o ${HYBRID}.root\n\n")

            output.write("### Do the GroupBy\n")
            # output.write( buildFolder + "simReadout -c $CONFIG_F -f $FILTERS -i $INPUT_F -o $OUTPUT_F1\n\n")
            output.write( buildFolder + "simReadout -c $CONFIG_F -i ${HYBRID}.root -o $OUTPUT_F1\n\n")
            output.write("### Do the pulse triggering\n")
            if (peakFracs):
                for peakFrac in peakFracs:
                    output.write( buildFolder + "ApplyCFD   -c $CONFIG_F -i ${OUTPUT_F1}.root -o ${OUTPUT_F2}_CFD" + str(float(peakFrac)) + " -t " + str(float(peakFrac)) + "\n")
            else:
                output.write( buildFolder + "ApplyCFD   -c $CONFIG_F -i ${OUTPUT_F1}.root -o $OUTPUT_F2\n")

            if deleteFiles == True:
                output.write("\n")
                output.write("### Delete the grouped files\n")
                output.write("rm ${HYBRID}.root\n")
                output.write("rm ${OUTPUT_F1}.root\n")

            ### No more pruning, a better and more efficient way must be found. Maybe dedicated jobs.
            # output.write("### Pruning of the trees\n")
            # output.write( buildFolder + "pruneSigFormOutput -i " + baseFolderOut + "\n")
        ...
        with open(argsFile, "w") as output:
            for i, inputFile in enumerate(inputfiles):
                ...
                output.write("%s " % calibration)                                               # CALIBRATION=$5
