...
def main(args):
    ...
    # parser.add_argument("--baseFolderOut",    # name on the CLI - drop the `--` for positional/required parameters
    #                     nargs="*",  # 0 or more values expected => creates a list
    #                     type=str,
    #                     default=[],
    #                     help='Folders with the output files',
    #                     required=True
    # )
    # parser.add_argument("--baseFolderIn",    # name on the CLI - drop the `--` for positional/required parameters
    #                     nargs="*",  # 0 or more values expected => creates a list
    #                     type=str,
    #                     default=[],
    #                     help='Folders with the input files',
    #                     required=True
    # )
    ...
    parser.add_argument('--queue'   , default='longlunch')
    parser.add_argument('--outhybrid'   , default='hybrid')
    parser.add_argument('--output1'   , default='OutGroupd')
    parser.add_argument('--output2'   , default='OutTrigd')
    ...
    parser.add_argument('--inputFilePrefix'   ,  default='out', help='Prefix of the input data files', required=True)
    ...
    for inputFolder in inputFolderList:
        ...
        with open(run_script, "w") as output:
            ...
            output.write("OUTPUT_F2=$4\n")
            output.write("CALIBRATION=$5\n")
            output.write("HYBRID=$6\n")
            output.write("SEED=$7\n")
            ...
            output.write("### Do the propagation\n")
            output.write(buildFolder + "propagateHybrid -i $INPUT_F -c \"$CALIBRATION\" --photonSeed $SEED -o hybrid_temp.root --type ")
            for tIndex in range(len(listTypes)):
                ...
                if tIndex != (len(listTypes)-1):
                    ...
                    output.write(" ")
            if external is not None:
                output.write("--external %s " %external)
            output.write("\n\n")
            output.write("### Do the GroupBy\n")
            # output.write( buildFolder + "simReadout -c $CONFIG_F -f $FILTERS -i $INPUT_F -o $OUTPUT_F1\n\n")
            output.write( buildFolder + "simReadout -c $CONFIG_F -i hybrid_temp.root -o output_f1\n\n")
            output.write("### Do the pulse triggering\n")
            outputf2_name = ""
            if (peakFracs):
                for peakFrac in peakFracs:
                    outputf2_name = "output_f2_CFD" + str(float(peakFrac))
                    output.write( buildFolder + "ApplyCFD   -c $CONFIG_F -i output_f1.root -o " + outputf2_name + " -t " + str(float(peakFrac)) + "\n")
            else:
                outputf2_name = "output_f2"
                output.write( buildFolder + "ApplyCFD   -c $CONFIG_F -i output_f1.root -o " + outputf2_name + " \n" )

            output.write("### Copy pulse files\n")
            output.write("xrdcp --nopbar %s.root ${OUTPUT_F2}.root\n" %outputf2_name)
            if deleteHybrid == False:
                output.write("\n")
                output.write("### Copy the Hybrid files\n")
                output.write("xrdcp --nopbar hybrid_temp.root ${HYBRID}.root\n")
            if deleteSignal == False:
                output.write("\n")
                output.write("### Copy the Signal files\n")
                output.write("xrdcp --nopbar output_f1.root ${OUTPUT_F1}.root\n")
        ...
        with open(argsFile, "w") as output:
            for i, inputFile in enumerate(inputfiles):
                ...
                output.write("%s " % (currentFolderIn  + inputFile) )                           # INPUT_F=$2
                output.write("%s " % (currentFolderOut + output1 + "_" + inputsuffix[i]) )      # OUTPUT_F1=$3
                output.write("%s " % (currentFolderOut + output2 + "_" + inputsuffix[i]) )      # OUTPUT_F2=$4
                for cIndex in range(len(listCalibrations)):
                  ...
                  if cIndex != (len(listCalibrations)-1):
                    ...
                    output.write(" ")                                                # CALIBRATION=$5
                output.write("%s " % (currentFolderOut + outhybrid + "_" + inputsuffix[i]) )    # HYBRID=$6
                output.write("%s " % (i+startOfRandom) )                                                    # SEED=$7
